...
class EmailCheck extends StatefulWidget {
  ...
  EmailCheck(
      {Key? key, required this.email, required this.pswd, required this.from, required this.camera}) // コンストラクタに camera を追加
      : super(key: key);
              ...
              onPressed: () async {
                ...
                if (_result?.user?.emailVerified ?? false) {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => HomeScreen(camera: widget.camera), // 修正: HomeScreen に変更
                    ),
                  );
                } else {
                  setState(() {
